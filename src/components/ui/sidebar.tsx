      ...
      >
        ...
        >
          <div
            data-sidebar="sidebar"
            className={cn(
              "flex h-full w-full flex-col overflow-hidden rounded-none bg-black text-sidebar-foreground",
              variant === "floating" && "rounded-lg border border-zinc-800",
...
>(({ className, ...props }, ref) => {
  ...
  return (
    <Button
      ...
      size="icon"
      className={cn(
        "hover:bg-zinc-800/50 text-white h-12 w-12",
        className
      )}
...
>(({ className, size, tooltip, asChild = false, ...props }, ref) => {
  const Comp = asChild ? Slot : "button"
  ...
  return (
    <TooltipProvider>
      <Tooltip>
        ...
        {tooltip && (
          <TooltipContent
            className="rounded-md bg-black px-2 py-1 text-xs font-semibold text-white font-['Menlo']"
            side="right"
            sideOffset={20}
          >
            {tooltip}
